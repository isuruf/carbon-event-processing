...
public class CarbonEventReceiverService implements EventReceiverService {
    ...
    public EventReceiverConfiguration getActiveEventReceiverConfiguration(String eventReceiverName) {
        ...
        Map<String, EventReceiver> tenantSpecificEventReceiverMap = this.tenantSpecificEventReceiverMap.get(tenantId);
    ...
    public void addEventReceiverConfiguration(EventReceiverConfiguration eventReceiverConfiguration)
        ...
        tenantSpecificEventReceiverMap.put(tenantId, eventReceiverConfigurationMap);
    }
            ...
            for (EventReceiverConfigurationFile eventReceiverConfigurationFile : eventReceiverConfigurationFileList) {
                if ((eventReceiverConfigurationFile.getFileName().equals(fileName))) {
                    if (eventReceiverConfigurationFile.getStatus().
                        ...
                        EventReceiver eventReceiver = tenantSpecificEventReceiverMap.get(tenantId).remove(eventReceiverName);
                        if (eventReceiver != null) {
    ...
    public void deactivateActiveEventReceiverConfigurationsForAdapter(
        ...
        List<EventReceiverConfigurationFile> fileList = new ArrayList<EventReceiverConfigurationFile>();
        if (tenantSpecificEventReceiverMap != null && tenantSpecificEventReceiverMap.size() > 0) {
            for (Map.Entry<Integer, Map<String, EventReceiver>> eventPublisherMapEntry : tenantSpecificEventReceiverMap.entrySet()) {
    ...
    public void deactivateActiveEventReceiverConfigurationsForStream(String streamId, int tenantId)
        ...
        List<EventReceiverConfigurationFile> fileList = new ArrayList<EventReceiverConfigurationFile>();
        if (tenantSpecificEventReceiverMap != null && tenantSpecificEventReceiverMap.size() > 0) {
            Map<String, EventReceiver> eventReceiverMap = tenantSpecificEventReceiverMap.get(tenantId);
...
//
//    @Override
//    public List<String> getSupportedInputMappingTypes(String eventAdapterName, int tenantId) {
...
//        InputEventAdapterManagerService InputEventAdapterManagerService = EventReceiverServiceValueHolder.getInputEventAdapterManagerService();
//        String eventAdapterType = null;
//        try {
//            InputEventAdapterConfiguration InputEventAdapterConfiguration =
//                    InputEventAdapterManagerService.getActiveInputEventAdapterConfiguration(eventAdapterName, tenantId);
//            eventAdapterType = InputEventAdapterConfiguration.getType();
//        } catch (InputEventAdapterManagerConfigurationException e) {
//            log.error("Error while trying to retrieve supported input mapping types.", e);
//        }
//        if (eventAdapterType != null) {
//            InputEventAdapterDto InputEventAdapterDto = InputEventAdapterService.getEventAdapterDto(eventAdapterType);
//            if (InputEventAdapterDto != null) {
//                for (String messageType : InputEventAdapterDto.getSupportedMessageTypes()) {
...
//        try {
//            InputEventAdapterManagerService inputEventAdapterManagerService = EventReceiverServiceValueHolder.getInputEventAdapterManagerService();
//            String transportAdapterName = inputEventAdapterManagerService.getDefaultWso2EventAdapter(axisConfiguration);
//            EventReceiverConfiguration defaultEventReceiverConfiguration =
//                    EventReceiverUtil.createDefaultEventReceiver(streamId, transportAdapterName);
...
//            }
//        } catch (InputEventAdapterManagerConfigurationException e) {
//            throw new EventReceiverConfigurationException("Error retrieving default WSO2 event adapter :" + e.getMessage(), e);
...
//        try {
//            InputEventAdapterManagerService inputEventAdapterManagerService = EventReceiverServiceValueHolder.getInputEventAdapterManagerService();
//            String defaultWso2EventAdapterName = inputEventAdapterManagerService.getDefaultWso2EventAdapter();
//            InputEventAdapterConfiguration defaultInputEventAdapter = inputEventAdapterManagerService.getActiveInputEventAdapterConfiguration(defaultWso2EventAdapterName, tenantId);
//            if (defaultInputEventAdapter != null) {
//                EventReceiverConfiguration defaultEventReceiverConfiguration =
//                        EventReceiverUtil.createDefaultEventReceiver(streamId, defaultWso2EventAdapterName);
//
//                String filename = defaultEventReceiverConfiguration.getEventReceiverName() + EventReceiverConstants.ER_CONFIG_FILE_EXTENSION_WITH_DOT;
//
//                if (!EventReceiverConfigurationFileSystemInvoker.isFileExists(filename)) {
...
//
//                        //TODO - We have to decide about default receiver handeling -- If there is a receiver which send events to the specific stream then there is no default receiver created
//                        //eventReceiverConfiguration.getInputStreamConfiguration().getInputEventAdapterName().equals(defaultWso2EventAdapterName) &&
//                        if ((eventReceiverConfiguration.getToStreamName() + ":" + eventReceiverConfiguration.getToStreamVersion()).equals(streamId)) {
//                            log.info("Skipping defining default event receiver " + defaultEventReceiverConfiguration.getEventReceiverName() + " as " + eventReceiverConfiguration.getEventReceiverName() + " already exist");
//                            return;
//                        }
//                    }
